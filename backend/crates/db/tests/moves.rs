...
async fn prune_move_history_removes_only_past_cutoff(pool: PgPool) {
    ...
    assert_eq!(removed, 1);
    // fresh만 남는다.
    let rows = moves::history(&pool, None, None, 10).await.unwrap();
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0].file_id, fresh.file_id);
}

#[sqlx::test(migrations = "./migrations")]
async fn history_filters_by_file_id_and_outcome(pool: PgPool) {
    wire(&pool).await;
    let a = active_file(&pool, 100).await;
    let b = active_file(&pool, 100).await;
    moves::insert_move(&pool, a.file_id, "s", "d", &a.object_key)
        .await
        .unwrap();
    moves::finish_move_with_history(&pool, a.file_id, "moved")
        .await
        .unwrap();
    moves::insert_move(&pool, b.file_id, "s", "d", &b.object_key)
        .await
        .unwrap();
    moves::finish_move_with_history(&pool, b.file_id, "canceled")
        .await
        .unwrap();
    let only_a = moves::history(&pool, Some(a.file_id), None, 10)
        .await
        .unwrap();
    assert_eq!(only_a.len(), 1);
    assert_eq!(only_a[0].file_id, a.file_id);
    // outcome 필터는 그 결과만 낸다.
    let canceled = moves::history(&pool, None, Some("canceled"), 10)
        .await
        .unwrap();
    assert_eq!(canceled.len(), 1);
    assert_eq!(canceled[0].file_id, b.file_id);
    // 전체는 둘 다.
    assert_eq!(
        moves::history(&pool, None, None, 10).await.unwrap().len(),
        2
    );
}
...
#[sqlx::test(migrations = "./migrations")]
async fn admin_list_files_surfaces_in_flight_move(pool: PgPool) {
    wire(&pool).await;
    let idle = active_file(&pool, 100).await;
    let moving = active_file(&pool, 100).await;
    moves::insert_move(&pool, moving.file_id, "s", "d", &moving.object_key)
        .await
        .unwrap();
    let rows = moves::admin_list_files(&pool, Some("s"), None, None, None, 10)
        .await
        .unwrap();
    let in_flight = |file_id| {
        rows.iter()
            .find(|row| row.file_id == file_id)
            .and_then(|row| row.in_flight_move.clone())
    };
    assert_eq!(in_flight(moving.file_id).as_deref(), Some("requested"));
    assert_eq!(in_flight(idle.file_id), None);
}
...
async fn admin_file_detail_includes_location(pool: PgPool) {
    ...
    // 아직 읽은 적 없으면 idle 신호는 None.
    assert!(detail.last_read_at.is_none());
    // 없는 파일은 None.
    assert!(
