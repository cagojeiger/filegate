//! write lease 갱신 ↔ 만료 중단의 경계 — 갱신은 살아 있는 lease에만 성립하고,
...
async fn reclaim_candidate(pool: &PgPool) -> files::AbortCandidate {
...
#[sqlx::test(migrations = "./migrations")]
async fn aborted_lease_cannot_be_extended(pool: PgPool) {
    wire(&pool).await;
    let file = create_ok(&pool).await;
    force_expire(&pool, file.file_id).await;
    let candidate = reclaim_candidate(&pool).await;
    assert!(files::finalize_abort(&pool, &candidate).await.unwrap());
    assert_eq!(file_state(&pool, file.file_id).await, "aborted");
