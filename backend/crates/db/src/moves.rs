...
/// files+locations 조인 한 줄 — API 핸들러의 검증 입력. declared_size는
/// 단일 PUT 복사 상한(5GiB) 가드가 본다.
#[derive(Debug, sqlx::FromRow)]
...
/// 운영자 파일 목록 한 줄 — files+locations 조인 (admin files API). idle
/// 신호(last_read_at)와 진행 중 이동 상태(in_flight_move)를 함께 낸다 —
/// 운영자가 이동 대상을 고르는 입력이다.
#[derive(Debug, sqlx::FromRow)]
pub struct AdminFileRow {
    ...
    /// 마지막 읽기 대여 시각 (lease_history) — 이동·드레인 우선순위의 idle 신호.
    pub last_read_at: Option<DateTime<Utc>>,
    /// 진행 중 이동이 있으면 그 state, 없으면 None.
    pub in_flight_move: Option<String>,
    pub created_at: DateTime<Utc>,
}

/// 운영자 파일 단건 상세 — files 전체 + 위치 + idle 신호 (admin files API).
#[derive(Debug, sqlx::FromRow)]
...
pub async fn location_of(
    ...
    sqlx::query_as(
        "SELECT f.state AS file_state, l.storage_id, l.object_key, f.declared_size \
...
/// 이동 이력 조회 (운영자) — file_id·outcome으로 좁히거나 전체. 최근순
/// (finished_at DESC).
...
/// LEFT JOIN이라 종착 파일(purge 완료)도 상태 필터가 부르면 뜬다. last_read_at은
/// lease_history의 마지막 읽기 대여(idle 신호), in_flight_move는 진행 중 이동의
/// state다. after가 있으면 그 파일의 (created_at, id) 뒤부터, (created_at, id)
/// 오름차순.
pub async fn admin_list_files(
    ...
    sqlx::query_as(
        "SELECT f.id AS file_id, f.client_id, f.state, f.declared_size, f.content_type, \
         l.storage_id, l.object_key, \
         (SELECT max(h.at) FROM lease_history h WHERE h.file_id = f.id AND h.kind = 'read') \
             AS last_read_at, \
         m.state AS in_flight_move, f.created_at \
         FROM files f \
         LEFT JOIN locations l ON l.file_id = f.id \
         LEFT JOIN object_moves m ON m.file_id = f.id \
         WHERE f.state = $1 \
...
/// 운영자 파일 단건 상세 — files 전체 + 위치 + idle 신호. location은 LEFT
/// JOIN이라 종착 파일도 뜬다 (storage_id·object_key는 None). 진행 중 이동은
/// 핸들러가 get_move로 따로 붙인다. 없으면 None.
pub async fn admin_file_detail(
    ...
    sqlx::query_as(
        "SELECT f.id AS file_id, f.client_id, f.state, f.declared_size, f.content_type, \
         f.etag, l.storage_id, l.object_key, \
         (SELECT max(h.at) FROM lease_history h WHERE h.file_id = f.id AND h.kind = 'read') \
             AS last_read_at, \
         f.created_at, f.committed_at, f.deleted_at \
