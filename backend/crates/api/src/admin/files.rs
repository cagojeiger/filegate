...
//! last_read_at(마지막 읽기 대여)이 idle 신호, in_flight_move가 이동 중 표시다.
...
struct FileOut {
    ...
    /// 마지막 읽기 대여 시각 (lease_history) — 이동 우선순위의 idle 신호.
    last_read_at: Option<DateTime<Utc>>,
    /// 진행 중 이동이 있으면 그 state, 없으면 null.
    in_flight_move: Option<String>,
...
impl From<moves::AdminFileRow> for FileOut {
    fn from(row: moves::AdminFileRow) -> Self {
        Self {
            ...
            in_flight_move: row.in_flight_move,
