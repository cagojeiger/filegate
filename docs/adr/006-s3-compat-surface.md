...
- Status: Deferred (v0.1.0 범위 밖)
- Date: 2026-07-14 (개정 2026-07-14: v0.1.0 범위에서 유보)
- 부모: [005](005-presigned-byte-plane.md) (네이티브 바이트 평면), [002](002-lease-model.md), [003](003-url-ownership.md)

> **유보 (2026-07-14):** S3 호환 표면은 기존 외부 S3 도구(무수정 SDK·aws-cli)를
> filegate에 그대로 태우기 위한 온보딩 계층이다. v0.1.0의 범위는 단독 실행과
> 자체 네이티브 클라이언트이며, 태울 외부 S3 도구가 없다 — 이 계층은 그 수요가
> 실재할 때 되살린다. 아래 결정과 근거는 그때의 출발점으로 보존한다.
...
- 호환 표면의 오퍼레이션 계약(에러 코드·헤더·응답 모양)은 이 표면을
  되살릴 때의 spec이 정의한다 (구현과 함께).
