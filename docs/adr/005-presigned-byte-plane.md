...
- **중계 URL은 presigned의 등가물이다.** 서명 능력이 없는 storage(fs)와
  ...
  두 번째 프로토콜이 아니다.
- **S3 호환 표면은 핫패스에 두지 않는다.** S3 GetObject의 계약은 "응답
...
- 도구·마이그레이션용 S3 호환 보조 표면(rclone·백업, 중계 비용 감수)은
  이 결정이 금지하지 않는다 — 도입은 별도 결정이며 "사용자 서빙 금지"를
  계약에 명시해야 한다.
- SDK가 아닌 HTTP 클라이언트(브라우저·curl)는 307을 따라가므로, 보조
  표면의 다운로드 오프로드는 클라이언트 종류에 따라 부분 성립한다.
