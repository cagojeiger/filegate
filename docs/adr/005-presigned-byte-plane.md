...
- Status: Accepted
- Date: 2026-07-13 (개정 같은 날: 인터페이스 계약으로 승격 — 보조 S3 표면 옵션 철회. 개정 2026-07-14: S3 호환 표면은 [ADR 006](006-s3-compat-surface.md)으로 분리해 유보(v0.1.0 범위 밖) — 이 문서가 v0.1.0 바이트 인터페이스의 전부다)
...
- **S3 호환 표면은 이 평면의 일부가 아니다.** S3 GetObject의 계약은
  ...
  실측은 유효하다 — 호환 표면은 [ADR 006](006-s3-compat-surface.md)으로 분리해
  유보한다(v0.1.0 범위 밖). v0.1.0의 바이트 인터페이스는 이 네이티브 평면 하나다.
...
- S3 호환 표면은 [ADR 006](006-s3-compat-surface.md)으로 분리해
  유보한다 (2026-07-14, v0.1.0 범위 밖) — 스파이크(`spike/s3-gateway`:
  SigV4 검증·논리키 매핑)를 그 재료로 보존한다.
