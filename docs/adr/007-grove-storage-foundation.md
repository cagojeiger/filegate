...
- 상태: Accepted (제품 방향)
- 날짜: 2026-09-08
- 구현 상태: 중앙 메타데이터·서버 로컬 fs·외부 S3 구현, CLI·관리 이관·Agent는 후속 작업
...
아래는 구현·운영 이관 전의 계획과 검증 기준이다. Accepted는 제품 방향의 채택을 뜻하며,
...
| 운영 | [기술·운영](../stack/README.md) | Terraform 대체 절차·배포 이름 전환 |

## 결과

후속 제품명은 **Grove Storage**다. 이번 1차에서는 저장소·바이너리·CLI·환경 변수·배포
이름을 FileGate로 유지하며, 실제 이름과 소비자 전환은 별도 릴리스에서 정한다.
