...
| **file / 배치(placement)** | 파일의 정체성 / 실물의 자리. 배치는 실물 하나당 하나이고 역할(정본·준비·버려짐)을 가진다 ([ADR 007](007-placement-lifecycle.md)) |
...
| **소프트 삭제 / purge** | 삭제의 두 단계. 소프트 삭제는 서비스의 결정, purge는 배치를 버려짐으로 넘기는 집행이다 |
| **tiering** | capacity 압박 시 파일의 배치를 옮기는 재배치. 정책이 생성하고 집행자가 옮긴다 |
...
| [007](007-placement-lifecycle.md) | 실물의 존재는 배치 행의 존재다 (finalizer 규율) | 공리 1 + ADR 001 |
