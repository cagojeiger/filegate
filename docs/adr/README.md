...
| 용어 | 뜻 |
|---|---|
| **등록부 (registry)** | storages·clients·credentials의 런타임 정본. DB에 살고 운영자 API로만 변경한다 |
| **클라이언트 (client)** | filegate를 쓰는 서비스의 등록 단위. 기반 storage 하나(`storage_id`)와 인증 수단을 소유하며, S3 호환 표면에서는 id가 버킷 이름이다 |
...
| **quota** | 향후 운영자가 클라이언트별로 정할 수 있는 용량 몫. 현재는 미구현이며, 도입해도 운영자 내부 가드레일로만 두고 클라이언트에게 노출하지 않는다 |
...
| [004](004-config-layers.md) | 서비스 신원과 배치는 client에, 정본은 DB에 둔다 (컨트롤 플레인) | 공유 전제 + 공리 1+3 |
