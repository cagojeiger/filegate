...
- 관찰은 동일: 사용량 집계의 단위는 declared_size 전체다 (spec 00 — 조회
  시점 파생). part는 회계 단위가 아니다.
...
- URL의 정체는 모드가 정한다 (서비스는 모른다): 직결이면 벤더 part presigned
  URL, 중계면 `/blobs/{lease}?s=…&part=N`.
- 중계 secret은 마스터 키 + lease id에서 결정적으로 파생한다 — 원문은
  저장되지 않고(해시만), 재발급이 곧 재파생이라 같은 URL이 나온다. 마스터
  키의 완전 회전(PREV 소거)을 지나면 그 이전 업로드의 secret은 재현 불가다
  — parts는 409를 내고, 업로드 재시작이 계약이다.
...
- 서비스는 part 목록을 제출하지 않는다. filegate가 자기 원장(중계: part
  ...
  spec 00의 commit과 동일하게 유지한다.) S3 호환 표면([spec 03](03-s3-surface.md))은
  프로토콜상 클라이언트가 Complete에 목록을 보내지만, filegate는 그것을 신뢰의
  근원이 아니라 **검증 입력**으로만 쓴다 — 같은 원칙이다.
...
- 중계 fs는 대상 임시 파일과 part 스풀을 지운다. part 행은 lease에 따라
  정리된다.
- 나머지(전이 우선, 멱등)는 spec 00의 회수와 같다.
...
- 파일 크기 상한 = `part_size × 10000` (벤더 part 수 한계의 반영). 벤더의
  절대 한계는 벤더가 직접 내고, 별도 운영자 상한은 두지 않는다.
