...
- ⚠️ **버킷 단위 CORS 설정 API가 없음** — 브라우저 직접 PUT(위임 업로드)이 성립하는지 M3에서 실측 필요. PAR 응답에 `access-control-allow-origin`이 온다는 커뮤니티 보고는 있으나 origin 제어 불가. 안 되면 폴백: 서버측 업로드 또는 presigned POST(브라우저 HTML 폼용으로 추가된 기능)
