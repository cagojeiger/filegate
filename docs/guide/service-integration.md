...
intent와 file_id만 알고, 물리(벤더·경로·수명)는 filegate가 소유한다.
...
storage(벤더 자격증명), client(서비스 신원 + 키), binding(intent → storage)을
등록한다 ([spec 01](../spec/01-registry.md)). 이후 서비스는 클라이언트 키
하나로 인증한다.

## 업로드

```
① POST /v1/files {intent, declared_size, content_type}
...
서술자를 준다. 전송 주체는 `POST /v1/files/{id}/parts {parts:[...]}`로
...
| 동사 | 호출 | 반환 |
|---|---|---|
| 다운로드 | `POST /v1/files/{id}/read` (선택: filename) | `{ get_url }` — 15분 만료, 전송 주체가 직접 GET |
| 조회 | `GET /v1/files/{id}` | `{ state, declared_size, intent }` |
| 삭제 | `DELETE /v1/files/{id}` | 삭제 결정 기록 — 물리 정리는 filegate 몫 |
...
삭제는 두 장부를 함께 정리한다: `DELETE /v1/files/{id}` + 서비스 행 삭제.

## 인증

| 표면 | 인증 |
|---|---|
| `/v1/*` | 클라이언트 키 (`Authorization: Bearer <key>`) |
| 바이트 URL | URL 자체가 자격 (서명·만료) — 추가 인증 없음 |
| `/admin/*` | 운영자 토큰 — 등록·상태·사용량 관찰은 운영자의 세계 |

## 체크리스트

- [ ] Terraform: storage + client + client_key + binding
