...
run_mode() {
  ...
  case "$URLKIND" in
    ...
    relay)  case "$PURL" in $BASE/blobs/*\?s=*) ok;; *) bad "[$INTENT] 중계 URL 아님: $PURL";; esac;;
  esac
  ...
  R=$(curl -s -H "$AUTH" -H "$JSON" -X POST $BASE/api/v1/files/$FID/read -d '{"filename":"모드 검증 v1+2&3#final.txt"}')
  ...
  expect "[$INTENT] 회계 active" "$SIZE" "$($PSQL "SELECT coalesce(sum(f.declared_size),0) FROM files f JOIN locations l ON l.file_id=f.id WHERE l.storage_id='$SID' AND f.state='active';" | tr -d ' ')"
  ST=$(curl -s -H "$AUTH" $BASE/api/v1/files/$FID)
...
C=$(curl -s -H "$AUTH" -H "$JSON" -X POST $BASE/api/v1/files -d '{"intent":"relay-att","declared_size":10}')
...
# CORS preflight
PF=$(curl -s -o /dev/null -D - -X OPTIONS "$BASE/blobs/$LEASE3" | tr -d '\r')
...
C4=$(curl -s -H "$AUTH" -H "$JSON" -X POST $BASE/api/v1/files -d "{\"intent\":\"fs-att\",\"declared_size\":9,\"declared_md5\":\"$WRONGMD5\"}")
...
expect "중계 md5 불일치 commit 400" 400 "$(curl -s -o /dev/null -w '%{http_code}' -H "$AUTH" -X POST $BASE/api/v1/files/$F4/commit)"
...
sleep 8
expect "회계 전부 0" "0" "$($PSQL "SELECT coalesce(sum(f.declared_size),0) FROM files f JOIN locations l ON l.file_id=f.id WHERE l.storage_id IN ('minio-relay','fs-local');" | tr -d ' ')"
